...
namespace VRSurgery.EditorTools
{
    ...
    public static class SceneStillCapture
    {
        ...
        public static int Capture()
        {
            ...
            try
            {
                // The player's real viewpoint. This used to be a hard-coded eye position carried
                // over from the old layout, which meant the capture kept showing a viewpoint the
                // player no longer occupies. It now reads the actual head camera in the scene.
                Camera head = Camera.main;
                Vector3 eye = head != null ? head.transform.position : new Vector3(0f, 1.65f, 0f);
                if (head == null)
                {
                    Debug.LogWarning("[Capture] No MainCamera; player_view falls back to a nominal eye position.");
                }

                Debug.Log($"[Capture] player_view from the real head pose {eye} looking at the field {field}");
                written += Shoot(camera, "player_view", 1280, 800, eye, field, 60f);
